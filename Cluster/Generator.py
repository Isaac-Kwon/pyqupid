...
3. ```GetDigitalSignal()``` 아날로그 시그널을 기준으로, ```Digitizer``` 에 정의된 디지털 그리드에 따라 해당 구역 내에 정의된 시그널 크기를 낸다. (그리드 센터의 값을 구한 다음에 면적을 곱하는 방식)
    * ```GetDigitalSignal_Old()``` 그리드 내부를 전부 적분하는 방식  
